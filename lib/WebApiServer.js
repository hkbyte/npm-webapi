        ...
        webApis.forEach((el) => {
            switch (el.method) {
                case enums_1.RequestMethod.POST:
                    this.server.post(el.endpoint, ...el.middlewares, el.integrate);
                    break;
                case enums_1.RequestMethod.GET:
                    this.server.get(el.endpoint, ...el.middlewares, el.integrate);
                    break;
                case enums_1.RequestMethod.DELETE:
                    this.server.delete(el.endpoint, ...el.middlewares, el.integrate);
                    break;
                case enums_1.RequestMethod.PATCH:
                    this.server.patch(el.endpoint, ...el.middlewares, el.integrate);
                    break;
                case enums_1.RequestMethod.PUT:
                    this.server.put(el.endpoint, ...el.middlewares, el.integrate);
                    break;
                case enums_1.RequestMethod.HEAD:
                    this.server.head(el.endpoint, ...el.middlewares, el.integrate);
                    break;
                default:
                    this.server.all(el.endpoint, ...el.middlewares, el.integrate);
            }
        });
    }
}
